...
\newcommand{\URSType}{\mathsf{URSType}}
...
satisfying the Unlinkability property (which implies \collisionResistance\!)
...
\begin{tabular}{@{\hskip 2em}l@{\;}l@{\;}l@{\;}l@{\hskip 0.25em}l}
...
and a type $\URSType$, we define a \term{family of group hashes into\, $\SubgroupG$}
as a function

\begin{formulae}
  \item $\GroupGHash{} \typecolon \URSType \times (\byteseq{8} \times \byteseqs) \rightarrow \SubgroupG$
\end{formulae}
...
\begin{nnotes}
  ...
  \item An alternative approach is to model $\GroupGHash{\URS}$ as a random
  ...
  \item The input element with type $\byteseq{8}$ is intended to act as a
        ...
        different purposes.
        Discrete Logarithm Independence is required to hold across choices of this parameter.
\end{nnotes}
} %sapling
...
\todo{explain these types}. One of these types is $\SIGHASHALL$\changed{, which is used for
...
\prenuzero{\todo{Describe the \sighash algorithm used prior to \NUZero activation,
i.e.\ for version 1 and 2 \transactions.}}
...
\begin{formulae}
  ...
  \item \tab where $l = \ItoLEBSPOf{6}{\MerkleDepthSapling - 1 - \mathsf{layer}}$.
\end{formulae}
...
\vspace{-2ex}
\securityrequirement{
$\DiversifyHash$ must be ``unlinkable'': given two randomly selected
...
\begin{nnotes}
  \item Suppose that $\GroupJHash{}$ acts as a random function from \diversifiers
        to points of order $\ParamJ{r}$ on the \jubjubCurve. Then unlinkability of
        $\DiversifyHash$ holds under the Decisional Diffie-Hellman assumption on the
        \jubjubCurve.
...
\begin{formulae}
  \item $\RedDSAHashToScalar(B) = \LEOStoIPOf{\RedDSAHashLength}{\RedDSAHash(B)} \!\pmod{\ParamG{r}}$
\end{formulae}
...
\sapling{
    ...
    \item Clarify the use of $\PHGR$ vs $\Groth$ proofs.
