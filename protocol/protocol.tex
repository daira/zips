...
\NewDocumentCommand{\citesub}{m}{%
  \saveexpandmode\noexpandarg
  \def\tempstring{#1}%
  \xStrSubstitute{\tempstring}{MAEA2010}{MAEÁ2010}[\tempstring]%
...
\newcommand{\NUFiveSpec}{NU5 proposal}
\newtoggle{issapling}
...
\notbeforenufive{Changes specific to the \NUFive proposal following \Canopy
are highlighted in \nufive{\nufivecolorname}.}
...
\nufive{and $\Uncommitted{Orchard} \typecolon$ $\bitseq{\MerkleHashLength{Orchard}}$,}
will also be defined in that section.
...
we can also derive a \diversifierKey $\DiversifierKey$, an \incomingViewingKey $\InViewingKey$,
an \outgoingViewingKey $\OutViewingKey$, and a set of \diversifiedPaymentAddresses
$\DiversifiedPaymentAddress = (\Diversifier, \DiversifiedTransmitPublic)$, as described in
...
If $\NoteCommitAlg{Orchard}$ returns $\bot$ (which happens with negligible probability),
...
\Mainnet \Canopy \activationBlock: $\mathtt{00000000002038016f976744c369dce7419fca30e7171dfac703af5e5f7ad1d4}$
...
\Testnet \Canopy \activationBlock: $\mathtt{01a4d7c6aada30c87762c1bf33fff5df7266b1fd7616bfdb5227fa59bd79e7a2}$
...
\nnote{$\NoteCommitAlg{Orchard}$ and $\CommitIvkAlg$ can return $\bot$ with negligible probability.}
...
\begin{nnotes}
  ...
  \item Under the Discrete Logarithm assumption on $\SubgroupG{}$, a \randomOracle almost surely satisfies
...
\begin{itemize}
  ...
  \item $\Spend$ for the \Sapling \spendStatement;
  \item $\Output$ for the \Sapling \outputStatement\notnufive{.}\notbeforenufive{;}
...
\begin{tabular}{@{\hskip 1.7em}r@{\;}l}
  $\AuthSignPrivate$ &$:= \ToScalar{Sapling}(\PRFexpand{\SpendingKey}([0]))$ \\
  $\AuthProvePrivate$ &$:= \ToScalar{Sapling}(\PRFexpand{\SpendingKey}([1]))$ \\
...
\begin{tabular}{@{\hskip 1.7em}r@{\;}l}
  ...
  \plap{$\InViewingKey$}{$\OutViewingKey$} &$:= \CRHivk\big(\reprJ\Of{\AuthSignPublic}, \reprJ\Of{\NullifierKey}\kern-0.08em\big)$.
\end{tabular}
...
\begin{nnotes}
  \item Assume that $\PRFexpand{}$ is a \xPRF with output range $\PRFOutputExpand$, where
        ...
        $f_{\sk}(t) := \ToScalar{Sapling}(\PRFexpand{\SpendingKey}(t))$.
...
Let $\PRFexpand{}$ and $\PRFock{Orchard}{}$, instantiated in \crossref{concreteprfs},
be \pseudoRandomFunctions.
...
Let $\DiversifyHash{Orchard}$, instantiated in \crossref{concretediversifyhash},
be a \hashFunction.
...
the \incomingViewingKey $\InViewingKey \typecolon \InViewingKeyTypeOrchard$,
and the \outgoingViewingKey $\OutViewingKey \typecolon \OutViewingKeyType$ are derived
as follows:

\begin{algorithm}
  \item let mutable $\AuthSignPrivate \leftarrow \ToScalar{Orchard}(\PRFexpand{\SpendingKey}([6]))$
  \item let $\NullifierKey = \ToBase{Orchard}(\PRFexpand{\SpendingKey}([7]))$
  \item let $\CommitIvkRand = \ToScalar{Orchard}(\PRFexpand{\SpendingKey}([8]))$
...
\begin{pnotes}
  \item The protocol does not prevent using the \diversifier $\Diversifier$ to produce
        ...
        since a vanity address can obviously be distinguished, and might leak more
        information than intended as to who created it.
  \item Similarly, address generators \MAY encode information in the \diversifierIndex
...
\begin{nnotes}
  ...
  \item The rule that $\cv$ and $\AuthSignRandomizedPublic$ \MUST not be small-order has the effect
        of also preventing non-canonical encodings of these fields\nufive{, as required by \cite{ZIP-216}}.
...
\begin{consensusrules}
  ...
  \item The proof $\Proof{\Output}$ \MUST be valid given a \primaryInput formed
        ...
        i.e.\ $\SpendVerify{}\big(\kern-0.1em(\cv, \cmU, \EphemeralPublic), \Proof{\Output}\big) = 1$.
\end{consensusrules}
...
Let $\GroupPx$ and $\ParamP{q}$ be as defined in \crossref{pallasandvesta}.
...
\begin{itemize}
  ...
  \item $\rt{Orchard} \typecolon \MerkleHash{Orchard}$ is an \anchor, as defined in
        \crossref{blockchain}, for the output \treestate of a previous \block;
  \item $\nf \typecolon \GF{\ParamP{q}}$ is the \nullifier for the input \note;
  ...
  \item $\cmX \typecolon \GF{\ParamP{q}}$ is the result of applying $\ExtractP$ to the \noteCommitment for
        the output \note;
...
\begin{nnotes}
  ...
  \item Despite the return type of $\ExtractP$ being $\GroupPx$, $\nf$ and $\cmX$ are \emph{not} checked
        to be in $\GroupPx$; they are only checked to be canonical encodings of $\GF{\ParamP{q}}$
        elements.
\end{nnotes}
} %nufive
...
\begin{itemize}
  ...
  \item Generate a \fullViewingKey $(\AuthSignPublic, \NullifierKey)$ and a
        \diversifiedPaymentAddress $(\Diversifier, \DiversifiedTransmitPublic)$
...
for both version 4 and version 5 \transactions, is defined in \cite{ZIP-244} as modified
by \cite{ZIP-225}. It will use a new \consensusBranchID \hexint{F919A198} as defined in
\cite{ZIP-252}.}
...
\begin{tabular}{@{\hskip 2em}r@{\;}l}
  $\BindingPublic{Orchard}$ &$= \Biggscalarmult{\Bigg(\!\vgrpsum{i=1}{n} \vNet{i}\Bigg) \grpminus \vBalance{Orchard}}{\ValueCommitValueBase{Orchard}}\, \combplus
                                \Biggscalarmult{\Bigg(\!\vgrpsum{i=1}{n} \ValueCommitRandNet{i}\Bigg)}{\ValueCommitRandBase{Orchard}}$ \\[3.5ex]
...
\begin{pnotes}
  ...
  \item $\SpendAuthSigRandomizePublic{Sapling}(\AuthSignRandomizer, \AuthSignPublic) = \AuthSignPublic + \scalarmult{\AuthSignRandomizer}{\AuthSignBase{Sapling}}$.
...
\begin{formulae}
  \item $\oparen\rt{Orchard} \typecolon \MerkleHash{Orchard},\\
         ...
         \hparen\nfOld{} \typecolon \GF{\ParamP{q}},\\
         \hparen\AuthSignRandomizedPublic \typecolon \SpendAuthSigPublic{Orchard},\\
         \hparen\cmX \typecolon \GF{\ParamP{q}},\vspace{0.2ex}\\
...
$\ExtractPbot\big(\NoteCommit{Orchard}{\NoteCommitRandNew{}}(\DiversifiedTransmitBaseNewRepr,
                                                             ...
                                                             \vNew{},
                                                             \NoteUniqueRandNew{},
                                                             \NoteNullifierRandNew)\kern-0.1em\big) \in \setof{\cmX, \bot}$, where
$\NoteUniqueRandNew{} = \nfOld{}$.
...
\begin{pnotes}
  ...
  \item The validity of $\DiversifiedTransmitBaseRepr$ and $\DiversifiedTransmitPublicRepr$ are
        \emph{not} checked in this circuit. Also, $\nfOld{}$ and $\cmX$ are \emph{not} checked to be in $\GroupPx$.
\end{pnotes}
...
\begin{nnotes}
  \item The procedure in \crossref{orchardkeycomponents} will always produce a \spendAuthAddressKey that
        ...
        other hand, allows the prover to witness $\AuthSignPublicPoint$ with $\tilde{y}$ set to $0$ or $1$.
  ...
  \item If $\NoteCommitAlg{Orchard}$ returns $\bot$ for the old or new \note, then the corresponding
        note commitment integrity check is satisfied. This models the fact that the implemented circuit
        uses incomplete addition to compute $\SinsemillaHashToPoint$. If an exceptional case were to occur,
        ...
        But as proven in \theoremref{thmincompletesinsemilla}, these exceptional cases allow immediately
        finding a nontrivial discrete logarithm, which is infeasible by assumption, and so finding such a
        case is infeasible.
\end{nnotes}
} %nufive
...
$\InViewingKeyTypeOrchard$ (in \Orchard)}} be the recipient's \incomingViewingKey, specified in
...
\nufive{
  ...
  \item if $\LEBStoOSP{256}\big(\ExtractG{}\big(\NoteCommitment{}\Of{\NoteTuple{}}\kern-0.1em\big)\kern-0.1em\big) \neq \cmstarField$, return $\bot$
        \vspace{-0.75ex}
...
\begin{pnotes}
  ...
  \item Normally only \noteCiphertextsSapling of \transactions in \blocks need to be decrypted. In that case,
        ...
        value can immediately be calculated as in \crossref{commitmentsandnullifiers}.
...
\begin{pnotes}
  ...
  \item A previous version of this specification did not have the requirement for the decoded point
        ...
        That did not match the implementation in \zcashd, which \emph{does} require $\DiversifiedTransmitPublic$
        to be in the subgroup. The specification has been changed to match \zcashd.
...
and \crossref{decryptivk}. \canopy{In particular:\!\!
\begin{itemize}
...
\vspace{-2ex}
$\MerkleCRH{Sprout}$ and $\MerkleCRH{Sapling}$ are used to hash
...
\begin{formulae}
  \item $\MerkleCRH{Sprout}(\mathsf{layer}, \mathsf{left}, \mathsf{right}) := \SHACompressBox{\merklebox}$.
\end{formulae}
...
\begin{formulae}
  \item $\MerkleCRH{Sapling}(\mathsf{layer}, \mathsf{left}, \mathsf{right}) := \PedersenHash(\ascii{Zcash\_PH},
           l \bconcat \mathsf{left} \bconcat \mathsf{right})$
  \item where $l = \ItoLEBSP{6}\big(\MerkleDepth{Sapling} - 1 - \mathsf{layer}\big)$.
\end{formulae}
...
\begin{nnotes}
  \item Suppose that $\GroupJHash{}$ (restricted to inputs for which it does not
        ...
        of $\DiversifyHash{Sapling}$ holds under the Decisional Diffie--Hellman assumption on the
...
\introlist
\vspace{2ex}
Define $\PedersenGenAlg{Sapling} \typecolon \byteseq{8} \times \Nat \rightarrow \SubgroupJstar$ by:
...
of the Discrete Logarithm Problem. It is designed by Sean Bowe and Daira Hopwood.
The motivation for introducing a new discrete-log-based hash function (rather than
...
\begin{tabular}{@{\hskip 1.5em}r@{\;}l}
  $\SinsemillaGenInit(D)$ &$:= \GroupPHash\!\big(\ascii{z.cash:SinsemillaQ}, D\big)$ \\
  $\SinsemillaGenBase(j)$ &$:= \GroupPHash\!\big(\ascii{z.cash:SinsemillaS}, \ItoLEOSPOf{32}{j}\big)$.
\end{tabular}
...
\begin{tabular}{@{\hskip 1.5em}r@{\;}l@{\;}l@{\;}l}
  $\bot$   &$\incompleteadd$ &$\bot$     &$= \bot$ \\[-0.5ex]
  $\bot$   &$\incompleteadd$ &$P$        &$= \bot$ \\[-0.5ex]
  $P$      &$\incompleteadd$ &$\bot$     &$= \bot$ \\[-0.5ex]
  $\ZeroP$ &$\incompleteadd$ &$\ZeroP$   &$= \bot$ \\[-0.25ex]
  $\ZeroP$ &$\incompleteadd$ &$(x', y')$ &$= \bot$ \\[-0.5ex]
  $(x, y)$ &$\incompleteadd$ &$\ZeroP$   &$= \bot$ \\[-0.25ex]
...
\begin{algorithm}
  \item pad $M$ to a multiple of $k$ bits by appending zero bits, giving $M'$.
  \item let $n \typecolon \range{0}{c} = \ceiling{\hfrac{\length(M')}{k}\kern-0.1em}$
  \item split $M'$ into $n$ \defining{\pieces} $M_\barerange{1}{n}$,
  ...
  \item for $i$ from $1$ up to $n$:
        \vspace{-1ex}
  \item \tab set $\Acc \leftarrow \Big(\Acc \incompleteadd \SinsemillaGenBase\big(\LEBStoIP{k}(M_i)\kern-0.1em\big)\kern-0.15em\Big) \incompleteadd \Acc$
  \item \blank
  \item return $\Acc$.
\end{algorithm}
...
\introlist
\theoremlabel{thmshortsinsemillacr}
...
Show that finding a collision $M, M' \typecolon \bitseq{\ell}$ with $M \neq M'$ such that
$\SinsemillaHashToPoint(D, M) = \SinsemillaHashToPoint(D, M')$ or $\SinsemillaHash(D, M) = \SinsemillaHash(D, M')$
efficiently yields a non-trivial discrete logarithm relation.
\end{theorem}

\begin{proof}
Without loss of generality we can restrict to the case where $\ell$ is a multiple of $k$,
since the zero padding is injective, and only has the effect of restricting the adversary's
choice of $M$ when $\ell$ is not a multiple of $k$.
...
to the discrete log problem in $\mathbb{P}$.

We collect the scalars by which each generator $\SinsemillaGenBase(j)$ is multiplied in the
algorithm for $\SinsemillaHashToPoint$. For a given $M$ define $m_i$ as in that algorithm.

For $j \in \binaryrange{k}$ define
\begin{formulae}
  \item $x_j = \sum \left\{ 2^{n-1-i} \text{ for } i \in \{0..n-1\} \text{ if } m_i = j \right\}$.
\end{formulae}

\theoremlabel{lemmasinsemillaonetoone}
\begin{lemma}[There is a $1:1$ mapping from $M$ to $x_{\barerange{0}{{2^k}-1}} \pmod{q}$]\end{lemma}

\begin{proof}
There is a $1:1$ mapping from $M$ to the matrix of bits with $2^k$ columns and $n$ rows, such
that the bit at column $j$ and row $i$ is set iff $m_i = j$. Then the binary representations of
$x_{\barerange{0}{{2^k}-1}}$ are given by the columns of this matrix, and they do not overflow
due to the requirement $2^n \leq \frac{q-1}{2}$. The claim follows. $\qed$
\end{proof}

Then we have
\begin{formulae}
  \item $\SinsemillaHashToPoint(M) = \scalarmult{2^n}{Q} + \vsum{j=0}{{2^k}-1} \scalarmult{x_j}{\SinsemillaGenBase(j)}$.
\end{formulae}

which is a Pedersen vector hash of the $x_i$'s, with a fixed offset $\scalarmult{2^n}{Q}$.
The fixed offset does not affect collision resistance in this context. (See below for why
it cannot be eliminated for $\SinsemillaHash$ or $\SinsemillaCommitAlg$, or when using incomplete
addition.) It follows that the collision resistance of $\SinsemillaHash$ can be tightly reduced,
via the proof in \cite[Appendix A]{BGG1995}, to the discrete log problem over $\mathbb{P}$.

Note that \cite{BGG1995} requires for their main scheme that the scalars are non-zero, which
is not necessarily the case in our context. However, their proof in Appendix A does not depend
on this, given that $n$ is fixed. The restriction that scalars are non-zero appears to have
been motivated by wanting to support variable-length messages and incremental hashing, which
we do not.

Now we consider $\SinsemillaHash$. We want to prove that if we can find two messages $M$ 
and $M'$ such that $\ExtractPbot\big(\SinsemillaHash(M)\kern-0.1em\big) =
\ExtractPbot\big(\SinsemillaHash(M')\kern-0.1em\big)$ then we can find a discrete logarithm.
So either $\SinsemillaHash(M) = \SinsemillaHash(M')$ (in which case use the original Pedersen
hash proof) or $\SinsemillaHash(M) = -\SinsemillaHash(M')$. In the latter case,

$\begin{array}{rcl}
\scalarmult{2^n}{Q} + \vsum{j=0}{{2^k}-1} \scalarmult{x_j}{P[j]} &=& -\left(\scalarmult{2^n} Q + \vsum{j'=0}{{2^k}-1} \scalarmult{x'_{j'}}{P[j'}\right) \\
\scalarmult{2^{n+1}} Q + \vsum{j=0}{{2^k}-1} \scalarmult{x_j + x'_j}{P[j]} &=& 0
\end{array}$
Because the coefficients $\!\!\pmod{q}$ are not all zero, this is a discrete log relation
between independent bases. This argument also extends straightforwardly to the binding
property of $\SinsemillaCommitAlg$.

$\SinsemillaCommitAlg$ is perfectly hiding because the output distribution is perfectly
indistinguishable from a random $\GroupP$ element, given that $r$ is a random scalar on
$[0, q)$. It follows that $\SinsemillaShortCommitAlg$ is also perfectly hiding, since hiding
cannot be affected by applying any fixed function to the \emph{output} of $\SinsemillaCommitAlg$.

\end{proof}

\nnote{
The above theorem covers the case where additional terms may be added to the
$\SinsemillaHashToPoint$ output before applying $\ExtractPbot$. This is needed
to show security of the $\SinsemillaShortCommitAlg$ \commitmentScheme defined in
\crossref{concretesinsemillacommit}. It is also needed to show security of the
...
\theoremlabel{thmincompletesinsemilla}
\begin{theorem}[A $\bot$ output from $\SinsemillaHashToPoint$ allows computing a nontrivial discrete logarithm]
...
\end{theorem}

\begin{proof}

We now aim to show that when Sinsemilla is instantiated over a \swCurve,
the additions can be implemented as incomplete additions.
...
$\textsf{Hash}(M)$:
    ...
    for $i$ from $0$ up to $n-1$:
        $A_{i+1} := (A_i + P[m_i]) + A_i$
    return $A_n$

We have an exceptional case iff $A_i = \pm P[m_i]$ or $A_i + P[m_i] = \pm A_i$. (Since none of $Q$, $P[0..2^k - 1]$ are $\mathcal{O}$, no intermediate results can be $\mathcal{O}$ unless one of the preceding conditions occurs.)

If $A_i + P[m_i] = A_i$, then we have $P[m_i] = \mathcal{O}$ contrary to assumption. So exceptional cases occur only if $[\alpha] A_i + P[m_i] = \mathcal{O}$ for some $i \in [0, n)$ and some $\alpha \in \{-1, 1, 2\}$.

$A_i$ has a representation $[2^i] Q + \sum\limits_{j=0}^{i-1} [x_j] P[j]$ for some $x_{0..i-1}$. So given $M$ that results in an exceptional case, the nontrivial discrete log relation $[\alpha 2^i] Q + \left(\sum\limits_{j=0}^{i-1} [\alpha x_j] P[j]\right) + P[i] = \mathcal{O}$ is easily computable from $M$. (The coefficients in this representation do not overflow since $|\alpha \cdot 2^i| \leq q-1$ for all $i < n$ and $\alpha \in \{-1, 1, 2\}$.)

Since by assumption it is hard to find a nontrivial discrete log relation, we can argue that it is safe to use incomplete additions when computing Sinsemilla inside a circuit. When computing the hash outside a circuit, we can either abort if an exceptional case occurs, or just compute it using complete formulae.
...
\vspace{-2ex}
\securityrequirement{
$\Poseidon \typecolon \GF{\ParamP{q}} \times \GF{\ParamP{q}} \rightarrow \GF{\ParamP{q}}$ must be a
...
\begin{nnotes}
  \item This construction of a PRF from a sponge is described in \cite[section 3.12]{BDPA2011}.
        ...
        However, the result of \cite{GG2015} does apply.
...
\begin{bytefield}[bitwidth=0.07em]{544}
    ...
    \sbitbox{256}{$\LEBStoOSPOf{256}{\ephemeralKey}$}
\end{bytefield}
\end{lrbox}
...
\nufive{
$\RedPallas$ is a specialization of $\RedDSA$ to the \pallasCurve (\crossref{pallasandvesta}),
using the $\BlakeTwob{512}$ hash function.
...
\vspace{-1ex}
The commitment scheme $\NoteCommit{Sprout}{}$ specified in \crossref{abstractcommit} is
...
The commitment scheme $\NoteCommitAlg{Sapling}$ specified in \crossref{abstractcommit} is
...
\introlist
The commitment scheme $\ValueCommitAlg{Sapling}$ specified in \crossref{abstractcommit} is
instantiated as follows using $\HomomorphicPedersenCommit{Sapling}{}$ on the \jubjubCurve:
\begin{formulae}
...
\nufive{
\introlist
The commitment scheme $\ValueCommitAlg{Orchard}$ specified in \crossref{abstractcommit} is
instantiated as follows using $\HomomorphicPedersenCommit{Orchard}{}$ on the \pallasCurve:
\begin{formulae}
...
The commitment scheme $\NoteCommitAlg{Orchard}$ specified in \crossref{abstractcommit} is
...
The commitment scheme $\CommitIvkAlg$ specified in \crossref{abstractcommit} is
instantiated as follows using $\SinsemillaCommitAlg$:
...
A \defining{\swEllipticCurve} over a field $\GF{q}$ for prime $q > 3$, as defined for example
in \cite[Definition 2.3.1]{Hisil2010}, is an elliptic curve $E$ over $\GF{q}$, parameterized
by $a, b \typecolon \GF{q}$ such that $4 \mult a^3 + 27 \mult b^2 \neq 0$, with equation
$E : y^2 = x^3 + a \mult x + b$. The curve has a distinguished zero point $\Zero$, also
...
background, see the notes for lectures 5, 6, and 7 at \cite{Sutherland2019}.}} to $\CurveP$; or
...
Precompute $\ParamIsoG{b} / (\ParamIsoG{Z} \mult \ParamIsoG{a})$.
...
\begin{algorithm}
  ...
  \item let $[\,u_0, u_1\,] = \hashtofield_{\XMDBlakeTwob}^{\typeexp{\GF{\ParamG{q}}\!}{2}}(\msg, \DST)$
...
\defining{\HaloTwo} \provingSystem described in \todo{}.
...
for a \Sapling\nufive{ or unified} \paymentAddress is to convert the \Bech\nufive{ or \Bechm}
form to uppercase and use the Alphanumeric mode \cite[sections 7.3.4 and 7.4.4]{ISO2015}.
} %sapling
...
if it is a non-canonical encoding as defined in \crossref{abstractgroup}. This \MAY be
...
\nufive{The specifications of the \NUFive upgrade will be described in this document,
\cite{ZIP-216}, \cite{ZIP-224}, \cite{ZIP-225}, and \cite{ZIP-244}. The contents of
...
\hbadness=10000
\begin{tabularx}{1.26\textwidth}{|c|c|l|p{11.1em}|L|}
\hline
...
\vspace{-0.3ex}
\scalebox{0.86}{
\begin{tabularx}{1.16\textwidth}{@{\!\!}l@{\hskip 1em}X@{}}
$\ddagger$ & The fields \valueBalance{Sapling}, \anchorField{Sapling}, \vSpendProofsSapling,
\vSpendAuthSigs{Sapling}, \vOutputProofsSapling, and \bindingSig{Sapling} are present if and
only if $\nSpendsSapling + \nOutputsSapling > 0$. If \valueBalance{Sapling} is not present,
then $\vBalance{Sapling}$ is defined to be $0$. \\[-0.5ex]

$\mathsection$ & The fields \flagsOrchard, \valueBalance{Orchard}, \anchorField{Orchard},
\sizeProofsOrchard, \proofsOrchard, \vSpendAuthSigs{Orchard}, and \bindingSig{Orchard}
are present if and only if $\nActionsOrchard > 0$. If \valueBalance{Orchard} is not present,
then $\vBalance{Orchard}$ is defined to be $0$.
\end{tabularx}
} %scalebox
\end{center}
\vspace{-2ex}
\scalebox{0.86}{
\!\!\Transactionversion 5 does not support \joinSplitTransfers.
...
\begin{consensusrules}
  ...
  \overwinteronlyitem{The \transactionVersionNumber \MUST be $3$, and the \versionGroupID \MUST
        be $\hexint{03C48270}$.}
  \saplingonwarditem{\;The\, \transactionVersionNumber\kern0.25em \MUST\, be\, $4$,\; and\, the\, \versionGroupID\kern0.25em
...
\notbeforeheartwood{
  ...
  \item A \coinbaseTransaction for a \block at \blockHeight greater than $0$ \MUST have a script
        that, as its first item, encodes the \blockHeight as follows. Let $\heightBytes$ be the signed
        little-endian representation of the number, using the minimum number of bytes such that
        the most significant byte is $< \hexint{80}$. Then the encoding is the length of
        $\heightBytes$ encoded as one byte, followed by $\heightBytes$ itself. This matches the
        encoding used by \Bitcoin in the implementation of \cite{BIP-34} (but the description here
...
\nufive{
  \item The flags in \flagsOrchard{} allow a version 5 \transaction to declare that no funds are spent
        ...
        a strictly contracting consensus change) to effectively ``switch off'' non-zero-value transfers
        only on the relevant side.
        These note spending and creation consensus rules are specified as part of the \HaloTwo \Orchard
        Action Statement (\crossref{actionstatement}).
} %nufive
  \nufiveonwarditem{Because the \enableSpendsOrchard{} is set to $0$ in version 5 \coinbaseTransactions\ --which
        disables non-zero-valued \Orchard spends-- the $\valueBalance{Orchard}$ field of a \coinbaseTransaction
        must have a negative or zero value.}
...
\begin{itemize}
  ...
  \nufiveonwarditem{In version 5 \transactions, these fields have been added: $\nActionsOrchard$, $\vActionsOrchard$,
        $\flagsOrchard$, $\valueBalance{Orchard}$, $\anchorField{Orchard}$, $\sizeProofsOrchard$, $\proofsOrchard$,
        $\bindingSig{Orchard}$, and $\vSpendAuthSigs{Orchard}$.}
...
\vspace{-0.5ex}
Let $\spendAuthSig$ be the \spendAuthSignature for this \spendTransfer. In a version 4 \transaction this
is encoded in the $\spendAuthSigField$ field of the \spendDescription. In a version 5 \transaction,
\spendAuthSignatures in $\vSpendAuthSigs{Sapling}$ are in one-to-one correspondence with \spendDescriptions
in $\vSpendsSapling$.
...
\vspace{-1ex}
\nufive{$\dagger$ The $\anchorField{Sapling}$, $\zkproof$, and $\spendAuthSigField$ fields are only present in a
\spendDescription if the \transactionVersion is $4$. For version 5 \transactions, all \spendDescriptions share
...
$80$ & $\outCiphertext$ & \type{byte[80]} & A ciphertext component for the
...
\xSpendAuthSignatures in $\vSpendAuthSigs{Orchard}$ are in one-to-one correspondence with
\actionDescriptions in $\vActionsOrchard$.
...
\nufiveonward{
In \Orchard, the \nullifier is computed as
$\DeriveNullifier{\NullifierKey}(\NoteUniqueRand, \NoteNullifierRand, \cm)$
as described in \crossref{commitmentsandnullifiers}. This construction
combines elliptic curve cryptography and the $\Poseidon$-based $\PRFnf{Orchard}{}$
in a way that, for privacy properties, aims to provide defence in depth
against potential weaknesses in either. Resistance to Faerie Gold attacks, on
the other hand, depends entirely on hardness of the Discrete Logarithm Problem.
The $\NoteUniqueRand$ value of a \note created in a given \actionTransfer is obtained
from the \nullifier of the \note spent in that \actionTransfer; this ensures
(without any cryptographic assumption) that all $\NoteUniqueRand$ values of
\notes added to the \noteCommitmentTree are unique. Then, the \nullifier
derivation can be considered as computing a modified Pedersen commitment on
input that includes $\NoteUniqueRand$, so that the binding property of that
...
\collisionResistance because, assuming hardness of the Discrete Logarithm
Problem on the \pallasCurve, the security proof in \theoremref{thmshortsinsemillacr}
covers the case where the additional term is added.) Roadblock attacks are
not possible because $\NoteUniqueRand$ does not repeat for \notes in the
\noteCommitmentTree, and by a corresponding argument to \Sapling for \dummyNotes.
} %nufiveonward
...
commitment: \shaHash for \Sprout\sapling{, and $\PedersenHash$ for \Sapling}.
...
transactions into generalized \joinSplitTransfers (for \Sprout),
\sapling{or \spendTransfers and \outputTransfers (for \Sapling)}, and each
transfer always uses a \zkSNARKProof\!\!, \Zcash does not require the nesting.
...
commitment scheme for this use at a 128-bit security level, the overhead of
...
Ying Tong Lai, Kris Nuttycombe, Jack Gavigan, and Steven Smith.
...
Jakub Zalewski, Oana Ciobotaru, and no doubt others.
...
and also Anna Rose and Fredrik Harrysson for their work on the Zero Knowledge Podcast,
ZK Summits, and ZK Study Club. These efforts have enriched the zero knowledge
...
\begin{itemize}
\nufive{
    \item Correct some interim findings of NCC specification audit:
          \begin{itemize}
            ...
            \item Change the output types of $\NoteCommitAlg{Orchard}$ and $\CommitIvkAlg$ to
                  reflect that these can return $\bot$.
          \end{itemize}
...
\sapling{
    \item The subgroup check added to \crossref{decryptovk} for \Sapling in version 2021.1.17 was
          applied to the wrong variable ($\DiversifiedTransmitBase$, when it should have been
...
\sapling{
    ...
    \item The note about non-canonical encodings in \crossref{jubjub} gave incorrect values
...
construct the \commitmentScheme $\NoteCommitAlg{Sapling}$.
...
\begin{formulae}
  \item $[1, \Selectu(\AuthSignRandomizedPublic), \Selectv(\AuthSignRandomizedPublic),
          \Selectu(\cvOld{}), \Selectv(\cvOld{}), \LEBStoIPOf{\MerkleHashLength{Sapling}}{\rt{Sapling}},
...
\begin{formulae}
  \item $[1, \Selectu\Of{\cvNew{}}, \Selectv\Of{\cvNew{}},
          \Selectu\Of{\EphemeralPublic}, \Selectv\Of{\EphemeralPublic}, \LEBStoIPOf{\MerkleHashLength{Sapling}}{\cmU}]$
\end{formulae}
