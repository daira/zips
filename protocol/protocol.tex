...
satisfying the Unlinkability security property described in \crossref{abstractgrouphash}.
...
\vspace{-2ex}
\begin{securityrequirements}
  \item $\Sym$ must be one-time (INT-CTXT $\wedge$ IND-CPA)-secure \cite{BN2007}.
        \quotedterm{One-time} here means that an honest protocol participant will almost
        surely encrypt only one message with a given key; however, the adversary may make
        many adaptive chosen ciphertext queries for a given key.
  \saplingonwarditem{...}
...
it must be reasonble to model $\GroupGHash{\URS}$ as a random oracle.
} %securityrequirement
...
\begin{nnotes}
  ...
  \item $\GroupJHash{}$ is also used to instantiate $\DiversifyHash$ in \crossref{concretediversifyhash}.
        ...
        in the standard model, but in a model where $\GroupJHash{}$ is taken as a random oracle,
...
These specializations are referred to as $\JoinSplit$ for the \Sprout
\joinSplitStatement (with either $\PHGR$ and $\BNCurve$, or $\Groth$ and
...
Similarly, we omit key subscripts on $\SpendProve$,
...
From this \spendingKey, the \authSigningKey $\AuthSignPrivate \typecolon \GFstar{\ParamJ{r}}$ and
\authProvingKey $\AuthProvePrivate \typecolon \GF{\ParamJ{r}}$ are derived as follows:
...
$\InViewingKey \typecolon \InViewingKeyTypeSapling$ are then derived as:
...
\begin{consensusrules}
  ...
  \item The \spendAuthSignature{} \MUST be a valid $\SpendAuthSig$ signature over $\SigHash$
        using $\AuthSignRandomizedPublic$ as the public key, where $\SigHash$ is the \sighashTxHash
        of this \transaction using $\SIGHASHALL$ as defined in \crossref{sighash} ---
...
\begin{itemize}
  ...
  \item let $\reprJ$ be as defined in \crossref{jubjub};
...
Let $\OutViewingKey \typecolon \OutViewingKeyType$ be the \outgoingViewingKey of the
\paymentAddress from which the \note is being spent.
...
\begin{itemize}
  ...
  \item let $\cvField = \LEBStoOSP{\ellJ}\big(\reprJ(\cvNew{})\kern-0.12em\big)$
  \item let $\cmField = \LEBStoOSP{256}\big(\ExtractJ(\cmNew{})\kern-0.15em\big)$
  \item let $\ephemeralKey = \LEBStoOSPOf{\ellJ}{\reprJOf{\EphemeralPublic}}$
  \item let $\OutCipherKey = \PRFock{\OutViewingKey}(\cvField, \cmField, \ephemeralKey)$
  \item let $\OutPlaintext = \LEBStoOSPOf{512}{\reprJ(\DiversifiedTransmitPublicNew) \,\bconcat\, \ItoLEBSPOf{256}{\EphemeralPrivate}\kern-0.12em}$
...
Let $(\EphemeralPublic, \TransmitCiphertext{}, \OutCiphertext)$ be the \noteCiphertext
from the \outputDescription{}.

Let $\cvField$, $\cmField$, and $\ephemeralKey$ be those
fields of the \outputDescription (encoding the \valueCommitment, the $u$-coordinate
of the \noteCommitment, and $\EphemeralPublic$ respectively).
...
$(\AuthProvePublic, \InViewingKey)$, to obtain each \note sent to the corresponding
\paymentAddress, its \memo field, and its final status (spent or unspent).
...
\begin{nnotes}
  \item Suppose that $\GroupJHash{}$ is modelled as a random oracle from \diversifiers
        to points of order $\ParamJ{r}$ on the \jubjubCurve. In this model, Unlinkability
...
\introlist
\vspace{2ex}
$\PRFock{}$ is used in \crossref{} to derive the
\authSigningKey $\OutCipherKey$ used to encrypt an \outputCiphertext.
...
It follows that $\ExtractJ$ is injective on $\SubgroupJ$.
}
...
Let $\GroupGHashInput := \byteseq{8} \times \byteseqs$.
...
\begin{formulae}
  \item $P := \abstJOf{\LEOStoBSPOf{256}{\BlakeTwosOf{256}{D,\, \URS \bconcat\, M}}}$
...
Let $\reprJ$ be as defined in \crossref{jubjub}.
...
$\LEBStoOSPOf{256}{\reprJOf{\cv}\hairspace}$. \\ \hline
...
Let $\ExtractJ$ be as defined in \crossref{concretegrouphashjubjub}.
...
Least Authority, and Mary Maller.
...
\sapling{
    ...
    \item Model the group hash, and the use of $\BlakeTwosGeneric$ to construct the group hash, as random oracles.
          This appears to be unavoidable in order to allow proving unlinkability of $\DiversifyHash$.
    \item Rename $\mathsf{CRS}$ (Common Random String) to $\URS$ (\uniformRandomString), to
          ...
          on May~10--11, 2018.
    \item Add reference to \cite{Carroll1876}.
} %sapling
\end{itemize}
